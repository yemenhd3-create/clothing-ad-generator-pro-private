...
const state = vi.hoisted(() => ({ auth: { isAuthenticated: false, loading: false }, query: { isLoading: false, data: undefined as unknown, error: null as unknown, refetch: vi.fn() } }));
vi.mock('@/_core/hooks/useAuth', () => ({ useAuth: () => state.auth }));
vi.mock('@/const', () => ({ startLogin: vi.fn() }));
vi.mock('@/lib/trpc', () => ({
  trpc: {
    personal: { access: { useQuery: () => state.query } },
...
describe('PersonalAccessGate PWA install entry', () => {
  ...
  afterEach(() => { cleanup(); state.auth = { isAuthenticated: false, loading: false }; state.query = { isLoading: false, data: undefined, error: null, refetch: vi.fn() }; });
