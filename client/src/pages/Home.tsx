...
function WardrobeToolPanel({ tool, settings, disabled, onClose, onChange, onScaleCommit, onRefine }: { tool: Exclude<WardrobeTool, null>; settings: TemplateSettings; disabled: boolean; onClose: () => void; onChange: (patch: StudioAppearancePatch) => void; onScaleCommit: (value: number) => void; onRefine: () => void }) {
  ...
  return <section className="absolute z-20 rounded-2xl border border-primary/15 bg-white p-2 shadow-xl" style={panelPosition} aria-label={title}><div className="mb-1 flex items-center justify-between"><h3 className="text-sm font-black text-primary">{title}</h3><button type="button" onClick={onClose} className="rounded-lg px-2 py-1 text-xs font-black text-primary">إغلاق</button></div>{tool === 'backdrop' && <div className="grid grid-cols-4 gap-2">{backdrops.map(item => <button key={item.id} type="button" disabled={disabled} onClick={() => onChange({ productBackdrop: item.id })} className={`rounded-xl px-2 py-2 text-xs font-black ${(settings.productBackdrop || 'soft') === item.id ? 'bg-primary text-primary-foreground' : 'bg-secondary text-primary'}`}>{item.label}</button>)}</div>}{tool === 'shadow' && <div className="grid grid-cols-3 gap-2">{shadows.map(item => <button key={item.id} type="button" disabled={disabled} onClick={() => onChange({ productShadow: item.id })} className={`rounded-xl px-2 py-2 text-xs font-black ${(settings.productShadow || 'grounded') === item.id ? 'bg-primary text-primary-foreground' : 'bg-secondary text-primary'}`}>{item.label}</button>)}</div>}{tool === 'size' && <div className="flex items-center gap-3"><button type="button" disabled={disabled || currentScale <= PRODUCT_SCALE_MIN} onClick={() => onScaleCommit(clampProductScale(currentScale - PRODUCT_SCALE_STEP))} className="rounded-xl bg-secondary px-3 py-2 text-xs font-black text-primary">أصغر</button><Slider value={[currentScale]} min={PRODUCT_SCALE_MIN} max={PRODUCT_SCALE_MAX} step={PRODUCT_SCALE_STEP} disabled={disabled} onValueCommit={values => onScaleCommit(clampProductScale(values[0]))} aria-label="حجم المنتج" /><button type="button" disabled={disabled || currentScale >= PRODUCT_SCALE_MAX} onClick={() => onScaleCommit(clampProductScale(currentScale + PRODUCT_SCALE_STEP))} className="rounded-xl bg-primary px-3 py-2 text-xs font-black text-primary-foreground">أكبر</button></div>}{tool === 'overlay' && <FloatingOverlayEditor settings={settings} disabled={disabled} onChange={onChange} onClose={onClose} onLayerChange={setOverlayLayer} />}{tool === 'refine' && <div><p className="text-sm text-muted-foreground">افتح الممحاة والعصا والتحديد الحر لتعديل الحواف محلياً، ثم عد إلى القالب.</p><button type="button" disabled={disabled} onClick={onRefine} className="reference-primary mt-2 w-full"><Wand2 size={17} />فتح تعديل الصورة</button></div>}</section>;
}
