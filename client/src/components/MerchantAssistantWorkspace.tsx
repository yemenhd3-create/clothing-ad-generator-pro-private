...
export default function MerchantAssistantWorkspace({ profile, session, template, onCommitProfile, onCommitSession, onApplyCommands, onApplyArtwork, onRequestOnlineReply, onRestoreBackup, onClearProfile, onClearSession, onOpenUpdatedResult }: MerchantAssistantWorkspaceProps) {
  ...
  const downloadBackup = () => {
    ...
    addAssistantMessage('حُفظت نسخة محلية آمنة من الهوية وإعدادات القالب وذاكرة القائد. لا تشمل المفاتيح أو صورة الإعلان الجاري. يمكنك رفع ملف JSON إلى Google Drive أو الاحتفاظ به في هاتفك.');
  };
  ...
  return <section className="space-y-5" aria-label="القائد المحلي">
    ...
    <section className="reference-card p-5" aria-label="نسخ احتياطي للمشروع"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 text-primary" size={21} /><div><span className="text-xs font-bold text-primary">نسخة احتياطية شخصية مستقلة</span><h3 className="mt-1 font-black text-foreground">احفظ عملك في هاتفك أو Google Drive</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">يُنشأ ملف JSON محلي لإعدادات القالب وذاكرة القائد. لا يحتوي مفاتيح API أو جلسات دخول أو صورة الإعلان الجاري. يبقى صالحاً للاستيراد حتى إن لم تتوفر خدمة خارجية.</p></div></div><div className="mt-4 grid grid-cols-2 gap-2"><Button type="button" variant="outline" onClick={downloadBackup}>تنزيل نسخة</Button><Button type="button" onClick={() => { void shareBackup(); }}>مشاركة إلى Drive</Button></div><label className="mt-2 flex cursor-pointer items-center justify-center rounded-xl border border-primary/15 bg-secondary px-3 py-2 text-xs font-bold text-primary transition active:scale-95">استيراد نسخة محفوظة<input className="sr-only" aria-label="استيراد نسخة احتياطية" type="file" accept="application/json,.json" onChange={event => readBackup(event.target.files?.[0])} /></label></section>
